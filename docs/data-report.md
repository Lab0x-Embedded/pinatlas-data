...
**封装类别必须"名字 + 引脚编号形态"一起判**（实测两类漏判）：

| 型号 | 封装 | 名字判断 | 实际数据 | 正确处理 |
|---|---|---|---|---|
| STM32F723RCVx | `VQFPN68` | `QFN` 匹配不到 `QFPN` → unknown | 线性 1..68 | **quad**（68 % 4 = 0） |
| STM32WB1MMCHx | `LGA77` | 像网格 | 线性 1..77，**没有行列坐标** | unknown + 告警（按线性近似，需对照数据手册） |
